...
このプロジェクトkaisuiはHaskellとRustなどの異なる言語のプログラムを通信する時に、
C FFIなどのメモリ共有をする方法ではなく、
Erlang風のメッセージパッシングライブラリを用いて通信することを試みています。

既存のメッセージパッシングライブラリを相互に接続することで通信を実現することを目指します。
...
回避できない問題はパフォーマンスが悪化することですが、
それはある程度諦めることにします。
そんなにパフォーマンスを気にするなら全部Rustで書けば良いと主張しておきます。

## なぜ既存のメッセージパッシングライブラリを使うのか

ゼロから自分で書くのは自由にできて魅力的ですが、
機能性などを考えるとあまり良いものはできなさそうなので。
...
TCPなどのネットワーク通信機構を使って外部のアクターと通信が出来ることが必要。
...
C FFIなどのForeign Function Interfaceは使わなくて良いように設計します。
